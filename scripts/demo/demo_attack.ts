...
const user = Keypair.generate();
  ...
  console.log("\n🚩 Starting Attack Path Demo (Hidden Transfer)...");
  console.log("--------------------------------------------------");

  const simulator = new AttackSimulator();
  const res = simulator.generateScenario(Scenario.HIDDEN_TRANSFER);

  console.log("1. DECLARE: Simulating 'Safe' Swap...");
  ...
  await program.methods
    ...
    .signers([user])
    .rpc();
  console.log("✅ Transaction Declared. Preview claims it is safe.");
  ...
  console.log("\n3. EXECUTE: Transaction landed with HIDDEN TRANSFER...");
  console.log("Fingerprint Diff:");
  console.log("  EXPECTED Transfers:", JSON.stringify(res.expected.transfers, null, 2));
  console.log("  ACTUAL Transfers:  ", JSON.stringify(res.actual.transfers, null, 2));
  ...
  console.log(`⚠️ Violation: ${verification.violatedRule} - ${verification.evidence}`);

  await program.methods
    .verifyTx(verification.isMatch, new Uint8Array(32).fill(6))
    .accounts({
      bondAccount,
  ...
  console.log("\n5. SETTLE: Triggering Claim Payout...");
  await program.methods
    .settleTx()
    .accounts({
      bondAccount,
  ...
  console.log("🎉 RESULT: Structural trick detected. Bond paid out to user. (Status: ClaimPaid).");
  console.log("--------------------------------------------------\n");
}
